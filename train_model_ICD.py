...
from tensorflow.keras.layers import (
    GRU, LSTM, Dense, Dropout, Masking, Input, Concatenate
)
...
# Load ICD features if available
has_icd = 'X_icd_train' in data.files
if has_icd:
    X_icd_train = data['X_icd_train']
    X_icd_val = data['X_icd_val']
    X_icd_test = data['X_icd_test']
    icd_feature_names = data['icd_feature_names']
    n_icd = X_icd_train.shape[1]
...
print(f"  Temporal features ({len(feature_names)}): {list(feature_names)}")
if has_icd:
    print(f"  ICD features: {n_icd} hierarchical codes")
else:
    print(f"  ICD features: not available (temporal-only model)")
...
T = X_train.shape[1]   # timesteps
F = X_train.shape[2]   # temporal features

if has_icd:
    # ---- Dual-input architecture: GRU (temporal) + Dense (static ICD) ----
    # Branch 1: temporal vitals/labs → GRU
    temporal_input = Input(shape=(T, F), name='temporal_input')
    x = Masking(mask_value=0.0)(temporal_input)
    x = GRU(128, return_sequences=True, name='gru_1')(x)
    x = Dropout(0.3, name='dropout_gru_1')(x)
    x = GRU(64, name='gru_2')(x)
    x = Dropout(0.3, name='dropout_gru_2')(x)

    # Branch 2: static ICD-10 codes → Dense
    icd_input = Input(shape=(n_icd,), name='icd_input')
    y_branch = Dense(64, activation='relu', name='icd_dense_1')(icd_input)
    y_branch = Dropout(0.3, name='dropout_icd_1')(y_branch)
    y_branch = Dense(32, activation='relu', name='icd_dense_2')(y_branch)

    # Merge branches
    merged = Concatenate(name='merge')([x, y_branch])
    z = Dense(32, activation='relu', name='dense_1')(merged)
    z = Dropout(0.2, name='dropout_final')(z)
    output = Dense(1, activation='sigmoid', name='output')(z)

    model = Model(
        inputs=[temporal_input, icd_input],
        outputs=output,
        name='Bleeding_Prediction_GRU_ICD'
    )
    print("  Architecture: Dual-input (GRU + ICD Dense)")
else:
    # ---- Original single-input GRU ----
    model = Sequential([
    ...
    print("  Architecture: Single-input (GRU only)")
...
if has_icd:
    train_inputs = [X_train, X_icd_train]
    val_inputs = [X_val, X_icd_val]
else:
